---
title: "On-ramp"
sidebarTitle: "On-ramp"
---

`TonPayWidget` can offer users an on-ramp step inside the hosted TON Pay flow when the wallet needs funds to continue checkout.

## How it works

1. The user opens `TonPayWidget`.
1. TON Pay evaluates the current checkout state inside the hosted flow.
1. If wallet funding is needed and on-ramp is available for that session, the widget can present a top-up step.
1. After the top-up flow, the user can continue checkout in the same widget experience.

## Merchant integration

No dedicated on-ramp props are required in the host app. Render `TonPayWidget` with the checkout parameters and let the hosted flow decide when to surface on-ramp.

```tsx
import { TonPayWidget } from "@ton-pay/ui-react";

export function Checkout() {
  return (
    <TonPayWidget
      amount={25}
      chain="testnet"
      comment="Order #8451"
      currency="TON"
      itemTitle="Premium plan"
      recipientWalletAddress="<RECIPIENT_WALLET_ADDRESS>"
    />
  );
}
```

## Host app responsibilities

- render the widget trigger
- pass checkout parameters such as `amount`, `currency`, and `recipientWalletAddress`
- respond to lifecycle callbacks when needed

The hosted widget owns the on-ramp UI and the transition back into checkout.
