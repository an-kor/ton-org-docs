...
#### `verifySignature(payload, signature, apiSecret)`

Verify the `X-TonPay-Signature` header for webhook requests.

- `payload`: raw JSON string or parsed request body object.
- `signature`: `X-TonPay-Signature` header value.
- `apiSecret`: webhook secret from the TON Pay dashboard.
...
type WebhookPayload =
  ...
  | TransferRefundedWebhookPayload;
```

See [Webhooks](/ecosystem/ton-pay/webhooks) for verification and validation
examples.
