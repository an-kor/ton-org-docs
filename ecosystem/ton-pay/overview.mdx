...
The UI components render a payment button and open the hosted TON Pay widget. The host application passes checkout parameters such as amount, recipient wallet address, asset, and comment, while TON Pay handles session bootstrap, checkout routing, wallet connection, transaction signing, and result handling.

The UI components provide a complete checkout experience for most web applications. Choose the integration that fits your stack:

- **React** - [`TonPayWidget`](/ecosystem/ton-pay/ui-integration/button-react) component for React applications
